...
while true; do
    ...
    echo "${VERMELHO}""3 - Remover"${F_VERMELHO}""
    ...
    cadastro_usuario(){
        ...
        echo echo $nome $sobrenome,"${AMARELO}" cadastrado com sucesso !!"${F_AMARELO}" && sleep 2&& clear
    }
    ...
    remove_usuario(){
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $remover = "s" ]; then
                ...
                echo $nome,"${AMARELO}" REMOVIDO com sucesso !!"${F_AMARELO}" && sleep 2&& clear
            fi
        else
            echo "${VERMELHO}"USUARIO NÃO ENCONTRADO!! "${F_VERMELHO}""${VERDE}"Aguarde..."${F_VERDE}" && sleep 2 && clear
        fi
    ...
    Valida_cadastro() {
        ...
        if [ "$(grep -i $cad_nome "$ARQUIVO_DADOS")" ] && [ "$(grep -i $cad_sobrenome "$ARQUIVO_DADOS")" ]; then
            echo "${VERMELHO}"USUARIO JÁ CADASTRADO!! "${F_VERMELHO}""${VERDE}"Aguarde..."${F_VERDE}" && sleep 2 && clear
        fi
    }
